...
class FoodListAdapter : RecyclerView.Adapter<FoodListAdapter.FoodListViewHolder>() {
    ...
    inner class FoodListViewHolder(private val binding: FoodItemBinding) :
        ...
        fun bind(viewModel: Food) {
            binding.apply {
                priceTxt.text = "${viewModel.Price}$"
                timeTxt.text = viewModel.TimeValue.toString()
                titleTxt.text = viewModel.Title
                starTxt.text = viewModel.Star.toString()
                Glide.with(root).load(viewModel.ImagePath).into(img)
